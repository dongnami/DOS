...
class AttendAndExcitePipeline(
    ...
    def _update_latent(latents: torch.Tensor, loss: torch.Tensor, step_size: float) -> torch.Tensor:
        """Update the latent according to the computed loss using an in-place update."""
        # loss에 대해 gradient 계산 (create_graph=False로 불필요한 그래프 생성을 막음)
        grad_cond = torch.autograd.grad(loss, latents, create_graph=False)[0]
        # in-place로 업데이트 (data 속성을 직접 수정)
        latents.data.sub_(step_size * grad_cond)
        # 업데이트 후 이전 계산 그래프를 제거하고, 새롭게 gradient 추적을 시작하도록 함
    ...
    def _perform_iterative_refinement_step(self,
        ...
        while loss > target_loss:
            iteration += 1
            # 기존 계산 그래프를 버리기 위해 detach() 사용
            latents = latents.detach().requires_grad_(True)
            # forward pass (gradient 추적)
            ...
            # noise_pred_text는 loss 계산에 직접 사용되지는 않으므로 이후 바로 해제
            del noise_pred_text
            ...
            # forward inference without gradient 계산 (여기서 두 결과를 임시로 생성)
            with torch.no_grad():
            ...
            # 두 forward 결과가 이후 계산에 직접 사용되지 않으므로 바로 해제
            del noise_pred_uncond, noise_pred_text

            # losses로부터 낮은 attention 값을 갖는 토큰을 찾은 후 사용
            try:
            ...
            # 필요하지 않은 losses와 max_attention_per_index도, 이후 반복에서 새롭게 계산하므로 해제
            del losses  # losses 리스트가 더 이상 필요 없으므로
            # 만약 max_attention_per_index를 이후 사용하지 않고 단순 값만 출력했다면 del 후, 필요한 값만 복사해둘 수도 있음

            if iteration >= max_refinement_steps:
                print(f'\t Exceeded max number of iterations ({max_refinement_steps})! Finished with a max attention of {max_attention_per_index[low_token]}')
                break

        # 마지막 한 번의 forward pass (gradient 없이)
        latents = latents.detach().requires_grad_(True)
        noise_pred_text = self.unet(
            latents, t,
            encoder_hidden_states=text_embeddings[1].unsqueeze(0),
        ...
        loss, _ = self._compute_loss(max_attention_per_index, return_losses=True)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        indices_to_alter = kwargs.get("indices_to_alter", None)
        attention_res = kwargs.get("attention_res", 32) # 16
        thresholds = kwargs.get("thresholds", {0: 0.05, 10: 0.5, 20: 0.8})
        scale_factor = kwargs.get("scale_factor", 20) 
        scale_range = kwargs.get("scale_range", (1., 0.5))
        smooth_attentions = kwargs.get("smooth_attentions", True)
        sigma = kwargs.get("sigma", 0.5)
        kernel_size = kwargs.get("kernel_size", 3)
        max_iter_to_alter = kwargs.get("max_iter_to_alter", None)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if self.interrupt:
                    continue
                added_cond_kwargs = {"text_embeds": torch.zeros_like(add_text_embeds[1:]),
                    ...
                    if not run_standard_sd:
                        ...
                        if i in thresholds.keys() and loss > 1. - thresholds[i]:
                            ...
                            loss, latents, max_attention_per_index = self._perform_iterative_refinement_step(
                                ...
                                sigma=sigma,
                                kernel_size=kernel_size,
                                normalize_eot=True, ) # need to be checked
